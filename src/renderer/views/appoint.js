...
export function mount(root, app) {
  // A suggestion picked on the Officials screen arrives pre-filled.
  const suggestion = app.ui.pendingSuggestion;
  app.ui.pendingSuggestion = null;
  root.innerHTML = `<div class="page"><div class="page-head"><div><div class="eyebrow">Setup</div><h1>Appoint an Official</h1>
    <p>${suggestion ? 'Pre-filled from a suggestion. Review the details, then appoint.' : 'An Official owns an area, works in the background with Claude Code, and briefs you on the schedule you set.'}</p></div></div>
  ...
  if (suggestion) form.fill(suggestion);
  return {};
}
