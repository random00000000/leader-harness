...
- **Harness Engineer** (roadmap item 5): a template shown in the setup console only when the chosen folder is this repository (`inspectProject`: git root plus `Leader Harness - Wiki/Wiki Home.md` plus `AGENTS.md`). It sets the title "Head of Engineering, Leader Harness" and a remit to work the roadmap per AGENTS.md, one item per session, shipped by pull request. Settings: Ship authority, isolated copy, daily briefing at 08:00, work every 4 hours. The Official follows AGENTS.md because Claude Code loads `CLAUDE.md` (which imports AGENTS.md) from its working directory.
